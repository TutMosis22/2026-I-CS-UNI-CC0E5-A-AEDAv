...
// =====================================================
// LLNode: usa CRTP (mismo patrón que AVLTreeNode)
// para que m_next sea del tipo derivado directamente,
// sin necesidad de static_cast en LinkedList.
//
// NodeType = LLNode<T> (se pasa a sí mismo)
// =====================================================

template <typename T>
class LLNode {
protected:
    // CRTP: el tipo del puntero al siguiente nodo es
    // el propio LLNode<T>, no un alias indefinido
    using Node = LLNode<T>;

public:
    // value_type necesario por BaseTrait
    using value_type = T;
...
public:
    LLNode()
        : m_data(T()), m_ref(Ref()), m_next(nullptr) {}

    LLNode(T data, Ref ref)
        : m_data(data), m_ref(ref), m_next(nullptr) {}
...
struct AscendingLinkedListTrait
    : public BaseTrait<LLNode<T>, less<T>> {};

template <typename T>
struct DescendingLinkedListTrait
    : public BaseTrait<LLNode<T>, greater<T>> {};

// =====================================================
// LinkedList: recibe un Trait (estandarizado)
// Uso: LinkedList<AscendingLinkedListTrait<T1>> list;
// =====================================================
...
template <typename Trait>
void LinkedList<Trait>::internal_insert(
    Node* &pPrev, const value_type &value, Ref ref)
{
...
template <typename Trait>
std::tuple<typename LinkedList<Trait>::value_type, Ref>
LinkedList<Trait>::pop_front() {
    unique_lock<shared_mutex> lock(m_mtx);
