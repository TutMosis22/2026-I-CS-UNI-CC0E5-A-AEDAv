...
// =====================================================
// DemoList: inserta, imprime, graba y recarga una lista
// =====================================================
...
void TestConstructores() {
    ...
    LinkedList<AscendingLinkedListTrait<T1>> copia(original);
    cout << "Copy Constructor: " << copia << endl;

    LinkedList<AscendingLinkedListTrait<T1>> movida(std::move(copia));
    cout << "Move Constructor: " << movida << endl;

    LinkedList<AscendingLinkedListTrait<T1>> asignada;
    asignada = original;
    cout << "Copy Assignment: " << asignada << endl;

    LinkedList<AscendingLinkedListTrait<T1>> asignadaMove;
    asignadaMove = std::move(asignada);
    cout << "Move Assignment: " << asignadaMove << endl;
}
...
void TestPops() {
    ...
    cout << "Lista inicial: " << list << endl;

    auto front = list.pop_front();
    cout << "pop_front() -> ("
         << get<0>(front) << "," << get<1>(front) << ")" << endl;

    auto back = list.pop_back();
    cout << "pop_back() -> ("
         << get<0>(back) << "," << get<1>(back) << ")" << endl;
